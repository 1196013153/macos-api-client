...
覆盖 Java Controller 接口同步、变量解析、请求编排、JSON 解析、Mock 生成、form-data 与文件上传（multipart 逐字节）、编辑器定位与 JSON 高亮、项目模型、持久化往返、收藏置顶、接口搜索评分、按项目隔离的标签语义、环境语义色、工作区导入。不依赖 XCTest——零依赖才能在只有 Command Line Tools 的环境里跑（本机即如此）。
...
| `screenshot-project-tabs.png` | 单层标签条（全部项目的标签，按项目分组并加项目名前缀） |
...
- **全键盘**：`⌘F` 聚焦搜索框，`↑` `↓` 在结果里移动、`↵` 打开、`Esc` 清空并退出，手不用离开键盘
- **跨项目跳转**：`⌘K` 打开命令面板，搜**全部项目**的接口，回车直接切过去并打开——不用先切项目再搜；同样相关度下当前项目的结果排在前面

### 界面密度与环境护栏

请求区原先在正文之上堆了 6 条横条（项目标签、请求标签、地址、元信息、分区标签、正文类型），
加上响应区两条，窗口里约 **290px 垂直空间**用于「壳」，JSON 正文只剩 4 行左右。现在压到 **134px**：

- **元信息条整条删除**，内容按作用域各归其位：名称 / 星标 / 未保存 → 标签；
  解析后的真实地址 → 地址框内右对齐灰字（「我写的」和「实际发出的」同框对照）；
  环境 → 工具栏（它是项目级设置）；Mock 开关 → Mock 分区；保存 → `⌘S` + 标签脏点。
- **正文类型条并入分区标签行**右侧（那半行本来一直空着），响应区的状态码 / 耗时 / 大小同理并入响应分区行。
- 正文编辑器因此从约 150px 涨到约 306px，响应区多出约 52px。
- **环境色轨**：内容区顶部一条 2px 色带 + 工具栏色点，按环境名与 baseURL 推断——
  生产红、预发黄、测试绿。对着真实接口调试时「发错环境」是真实事故，一条色带是最低成本的护栏。
- 侧边栏树里的 HTTP 方法从 40px 文字徽标换成 3px 色轨，省下的横向宽度还给接口名。
...
```
┌ 请求标签（全部项目，按项目分组）──────────────────────────────┐
│ demo-api │ POST 账户余额 × │ demo-ai-brain │ POST 对话 ● × │ + │ ⋯ │
└─────────────────────────────────────────────────────────┘
```

- **一条就够**：早先顶上还有一条独立的项目标签行，那一行的信息量换不来一整行高度，已合并掉。项目名做成标签内前缀，只在同时打开多个项目时显示。
- 按项目列表顺序分组（不按打开顺序，来回切位置不会跳）；点别的项目的标签会**顺带切过去**并停在那个标签上。
- **每个项目仍各自记住自己停在哪个标签**：切项目回来还是原来那个，编辑缓冲、响应、编辑器位置全都不动。
- 标签上直接显示未保存圆点；**双击标签重命名**（名称原先在编辑区常驻一个输入框，属低频操作，改成按需弹出）。
- 快捷键：`⌘⇧[` / `⌘⇧]` 在同项目标签间切换，`⌘⌥[` / `⌘⌥]` 在项目间切换。
